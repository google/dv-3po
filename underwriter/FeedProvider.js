...
var FeedProvider = function(tabName, keys) {
  ...
  function applyChanges() {
    ...
    if(!keys) {
      ...
      _feed.forEach(function(feedItem) {
        var original = feedItem._original;
        var changes = {};

        result.push(feedItem);

        Object.getOwnPropertyNames(feedItem).forEach(function(propertyName) {
          if(propertyName[0] != "_") {
        ...
        feedItem._dups.forEach(function(dupFeedItem) {
          Object.getOwnPropertyNames(changes).forEach(function(propertyName) {
